...
public class ObjectController {
	...
	public void handleEvent(ObjectEvent objectEvent) {
		if (gameState== null) {
			TestGameState gameState =(TestGameState) GameStateManager.getInstance().getChild("game");
		}
//		if (objectEvent.getEventType() == EventTypes.OBJECT_CREATE) {
//			logger.info("Received event to create an object");
//			GameObject n = gameState.addObject(objectEvent.getObjectInfo());
...
//			GameObject deletedObject = ObjectStore.getInstance().deleteMovableObject(Integer.toString(objectEvent.getObjectInfo().getObjectID()));
