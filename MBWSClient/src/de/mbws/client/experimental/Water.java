...
public class Water {
	...
	public Water(String name, float xExtend, float yExtend, float xTextureSize,
			float yTectureSize, Vector3f location, Node rootNode) {
		waterQuad = new Quad(name, xExtend, yExtend);
		waterQuad.setLocalTranslation(location);
		height = location.y;

		Vector2f[] texCoords = { new Vector2f(0, 0),
				new Vector2f(xTextureSize, 0), new Vector2f(0, yTectureSize),
				new Vector2f(xTextureSize, yTectureSize) };

		waterQuad.reconstruct(waterQuad.getVertexBuffer(), waterQuad
				.getNormalBuffer(), null, BufferUtils
				.createFloatBuffer(texCoords), waterQuad.getIndexBuffer());
		waterQuad.setLocalTranslation(location);
	...
	// public Water(String name, float xExtend, float yExtend, float
	// xTextureSize, float yTectureSize, Vector3f location, Node rootNode) {
	// // waterQuad = new Quad(name, x, y);
	// // waterQuad.setLocalTranslation(location);
	// height = location.y;
	// waterMesh = new TriMesh("water");
	// // create the vertices
	// Vector3f[] vertices = { new Vector3f(0, 0, 0), new Vector3f(xExtend, 0,
	// 0),
	...
	// new ColorRGBA(0, 0, 0.8f, 0.5f), };
	//
	// Vector2f[] texCoords = { new Vector2f(0, 0), new Vector2f(xTextureSize,
	// 0),
