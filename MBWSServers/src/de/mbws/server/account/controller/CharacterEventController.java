...
public class CharacterEventController extends AccountServerBaseEventController {
	...
	public void handleEvent(AbstractGameEvent event) {
		...
		if (event.getEventType() == EventTypes.C2S_CHARACTER_RECEIVE_REQUEST) {
			...
			result
					.setEventType(EventTypes.S2S_CHARACTER_NEW_CHARACTER_ENTERS_WORLD);
			result.setPlayer(getAccountServer().getPlayerBySessionId(
					getAccountServer().getWorldServerSessionId()));
			sendEvent(result);
			...
			CharacterWorldServerInformation cwsi = (CharacterWorldServerInformation) event
					.getEventData();
			ServerRedirectEvent sre = new ServerRedirectEvent(
					((ServerCommunicationData) event.getPlayer()).getHostInfo());
			sre.setEventType(EventTypes.S2C_REDIRECT_TO_WORLDSERVER);
			sre.setPlayer(getAccountServer().getPlayerBySessionId(
					cwsi.getSessionId()));
			sendEvent(sre);
			...
			try {
				CharacterPersistenceManager.getInstance()
						.createCharacter(cdata);
	...
	private PlayerCharacterData getPlayerCharacterData(Characterdata cdata) {
		...
		playerData.setLocationdescription(cdata.getCharacterStatus().getMap()
				.getName());
