...
public class TerrainTest extends SimpleGame {
	...
	protected void simpleInitGame() {
		...
		t1 = TextureManager.loadTexture(getClass().getClassLoader().getResource(
				"resource/terrain-texture.jpg"), Texture.MM_LINEAR_LINEAR, Texture.FM_LINEAR);
		...
		for (int y = 0; y < SIZE; y++) {
			for (int x = 0; x < SIZE; x++) {
				int x1 = x - SIZE / 2;
				int y1 = y - SIZE / 2;
				 double z = Math.cos(Math.sqrt(x1 * x1 + y1 * y1) / 5.0) * 30.0;
		...
		terrain[0] = new Terrain(heightData, "Test");
		terrain[0].setLodQuality(0.7f);
		terrain[0].update(null);
		terrain[0].setTextures(t1, t2, 4, display);
		rootNode.attachChild(terrain[0]);
//		ImageIcon heightImage = new ImageIcon(getClass().getClassLoader().getResource(
//				"resource/terrain-heightmap.png"));
//		System.out.println(heightImage.getIconWidth());
//		AbstractHeightMap hm = new ImageBasedHeightMap(heightImage.getImage());
...
//				float horScale = 1.5f;
//				terrain[index] = new Terrain("test" + x + "_" + y, heightData, TERRAIN_SIZE,
//						new Vector3f(horScale, horScale, 0.1f), new Vector3f(horScale
//								* (TERRAIN_SIZE - 1) * y, horScale * (TERRAIN_SIZE - 1) * x, 0));
//				heightData = null;
//
