...
/**
 * sky gradient based on "A practical analytic model for daylight" by A. J.
 * Preetham, Peter Shirley, Brian Smits (University of Utah)
 * 
 * @author Highnik
 */
public class SkyDome extends Node {
  ...
  { 0.17872f, -1.46303f }, // a = darkening or brightening of the horizon
      ...
      { -0.02266f, 5.32505f }, // c = relative intensity of the circumsolar
                                // region
  ...
  public SkyDome(String name, Vector3f center, int planes, int radialSamples,
      float radius) {
    dome = new Dome(name, center, planes, radialSamples, radius, true);
    dome.setIsCollidable(false);
    //dome.setSolidColor(ColorRGBA.black);
    //dome.setLightCombineMode(LightState.REPLACE);
    Texture domeTexture = TextureManager.loadTexture(SkyDome.class.getClassLoader().getResource("jmetest/data/texture/cloud_land.jpg"),
    ...
    ZBufferState zbuff = DisplaySystem.getDisplaySystem().getRenderer()
        .createZBufferState();
  ...
  public void setSunPosition(float time) {
    ...
    // real time
    solarTime = time
        + (0.170f * FastMath.sin(4f * FastMath.PI * (julianDay - 80f) / 373f) - 0.129f * FastMath
            .sin(FastMath.TWO_PI * (julianDay - 8f) / 355f))
    ...
    opp = -cosSolarDeclination
        * FastMath.sin(FastMath.PI * solarTime / sunnyTime);

    adj = -(cosLatitude * sinSolarDeclination + sinLatitude
        * cosSolarDeclination
        * FastMath.cos(FastMath.PI * solarTime / sunnyTime));
    ...
    if (solarAltitude > 0.0f) {
      ...
      isNight = false;
      if ((opp < 0.0f && solarAzimuth < 0.0f)
          || (opp > 0.0f && solarAzimuth > 0.0f)) {
  ...
  public void update() {
    ...
    cameraPos = DisplaySystem.getDisplaySystem().getRenderer().getCamera()
        .getLocation();
  ...
  public void render() {
    ...
    for (int i = 0; i < dome.getBatchCount(); i++) {
      ...
      for (int j = 0; j < batch.getVertexCount(); j++) {
        ...
        yClear = perezFunctionO2(perezLuminance, cosTheta, gamma, cosGamma2,
            zenithLuminance);
  ...
  private float calc_sunny_time(float lat, float solarDeclin) {
    ...
    float sunnyTime;
    sunnyTime = (2.0f * FastMath.acos(-FastMath.tan(lat)
        * FastMath.tan(solarDeclin)));
  ...
  private void setupLensFlare() {
    ...
    sun = new LightNode("SunNode", DisplaySystem.getDisplaySystem()
    ...
    tex[0] = DisplaySystem.getDisplaySystem().getRenderer()
        .createTextureState();
    tex[0].setTexture(TextureManager
        .loadTexture(SkyDome.class.getClassLoader().getResource(
            "jmetest/data/texture/flare1.png"), Texture.MM_LINEAR_LINEAR,
            Texture.FM_LINEAR, Image.RGBA8888, 1.0f, true));
    tex[0].setEnabled(true);
    tex[0].apply();

    tex[1] = DisplaySystem.getDisplaySystem().getRenderer()
        .createTextureState();
    tex[1].setTexture(TextureManager.loadTexture(SkyDome.class.getClassLoader()
        .getResource("jmetest/data/texture/flare2.png"),
        Texture.MM_LINEAR_LINEAR, Texture.FM_LINEAR));
    tex[1].setEnabled(true);
    tex[1].apply();

    tex[2] = DisplaySystem.getDisplaySystem().getRenderer()
        .createTextureState();
    tex[2].setTexture(TextureManager.loadTexture(SkyDome.class.getClassLoader()
        .getResource("jmetest/data/texture/flare3.png"),
        Texture.MM_LINEAR_LINEAR, Texture.FM_LINEAR));
    tex[2].setEnabled(true);
    tex[2].apply();

    tex[3] = DisplaySystem.getDisplaySystem().getRenderer()
        .createTextureState();
    tex[3].setTexture(TextureManager.loadTexture(SkyDome.class.getClassLoader()
  ...
  private float perezFunctionO1(float[] perezCoeffs, float thetaSun,
    ...
    float val = (1.0f + perezCoeffs[0] * FastMath.exp(perezCoeffs[1]))
        * (1.0f + perezCoeffs[2] * FastMath.exp(perezCoeffs[3] * thetaSun) + perezCoeffs[4]
  ...
  private float perezFunctionO2(float[] perezCoeffs, float cosTheta,
    ...
    return zenithValue
        * (1.0f + perezCoeffs[0] * FastMath.exp(perezCoeffs[1] * cosTheta))
