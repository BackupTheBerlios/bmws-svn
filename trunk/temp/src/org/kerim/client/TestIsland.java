...
public class TestIsland extends SimplePassGame {
  ...
  private Timer timer;
  private static float terrainOffset=-11;
  ...
  protected void simpleUpdate() {
    ...
    dome.update();
    cam.update();
    Vector3f objectLocation=object.getLocalTranslation();
    float terrainHeight=tb.getHeight(objectLocation)+terrainOffset;
    if (objectLocation.getY()!=terrainHeight) {
      object.setLocalTranslation(new Vector3f(objectLocation.getX(),terrainHeight,objectLocation.getZ()));
  ...
  protected void simpleInitGame() {
    ...
    object.setLocalTranslation(0, tb.getHeight(new Vector2f(0, 0)), 0);
    ...
    projectedGrid = new ProjectedGrid("ProjectedGrid", cam, 50, 50, 0.01f,
        new HeightGenerator() {
          public float getHeight(float x, float z, float time) {
            return FastMath.sin(x * 0.0005f + time * 0.005f)
                + FastMath.cos(z * 0.1f + time * 4.0f) * 2;
          }
        });
  ...
  private void setupClouds() {
    Quad quad = new Quad("Q", 2000f, 2000f);
    quad.setLocalTranslation(new Vector3f(0,400,0));
    Matrix3f localRotate = new Matrix3f();
    localRotate.fromAngleAxis(-(1f * 0.5f * FastMath.PI), new Vector3f(1.0F,
        0.0F, 0.0F));
    quad.setLocalRotation(localRotate);
    
    final Texture texture = TextureManager.loadTexture(TestIsland.class.getClassLoader()
        .getResource("jmetest/data/texture//clouds.png"),
        Texture.MM_LINEAR, Texture.MM_LINEAR);
    ...
    quad.addController(new Controller() {
      ...
      public void update(float time) {
        texture.getTranslation().x += 0.005;// TRANSLATION_STEP;
  ...
  private void buildSkyBox() {
    ...
    Texture south = TextureManager.loadTexture(TestIsland.class
        ...
        Texture.FM_LINEAR);
    Texture east = TextureManager.loadTexture(TestIsland.class.getClassLoader()
        .getResource(dir + "2.jpg"), Texture.MM_LINEAR, Texture.FM_LINEAR);
    Texture west = TextureManager.loadTexture(TestIsland.class.getClassLoader()
        .getResource(dir + "4.jpg"), Texture.MM_LINEAR, Texture.FM_LINEAR);
    Texture up = TextureManager.loadTexture(TestIsland.class.getClassLoader()
        .getResource(dir + "6.jpg"), Texture.MM_LINEAR, Texture.FM_LINEAR);
    Texture down = TextureManager.loadTexture(TestIsland.class.getClassLoader()
  ...
  private Box createObjects() {
    object = new Box("object", new Vector3f(-0.5f, 0, -0.5f),
        new Vector3f(1, 2, 1));
    TextureState ts = display.getRenderer().createTextureState();
    Texture t0 = TextureManager.loadTexture(TestIsland.class.getClassLoader()
  ...
  private TerrainPage createIsland() {
    ...
    tb.updateModelBound();
    tb.setLocalTranslation(new Vector3f(0, terrainOffset, 0));
    return tb;
  }
  ...
  private void setupSkyDome() {
    dome = new SkyDome("skyDome", new Vector3f(0.0f, 0.0f, 0.0f), 11, 18,
        850.0f);
    ...
    // setup a target to LightNode, if you dont want terrain with light's effect
    // remove it.
    dome.setTarget(tb);
  ...
  private void createDebugQuads() {
    ...
    debugQuad.getLocalTranslation().set(quadWidth * 0.6f, quadHeight * 1.0f,
        1.0f);
    ...
    if (waterEffectRenderPass.getTextureRefract() != null) {
      ...
      debugQuad.setRenderState(ts);
      debugQuad.updateRenderState();
      debugQuad.getLocalTranslation().set(quadWidth * 0.6f, quadHeight * 2.1f,
          1.0f);
      debugQuadsNode.attachChild(debugQuad);
    }
  }
